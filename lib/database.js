...
var getUpdates = function(criteria, howMany, callback){
  this.find(criteria, {limit: howMany, sort: {created_time: -1}}, function(err, cursor){
    if (err) { callback(err); return; }
    cursor.toArray(function(err, docs){
      if (err) { callback(err); return; }
      callback(null, docs);
...
var getAllClocks = function(callback){
  this.find({type: 'clock'}, {}, function(err, cursor){
    if (err) { callback(err); return; }
    cursor.toArray(function(err, docs){
      if (err) { callback(err); return; }
      callback(null, docs.map(function(d){ return d.created_time; }));
    });
  });
};
