...
var fixNumericValues = function(data){
  var keysToFix = ['id', 'created_at'];
  if (data === null || data === undefined) {
    return data;
  } else if (typeof data.map === 'function') {
    return data.map(fixNumericValues);
  } else if (typeof data === 'object') {
    return Object.keys(data).reduce(function(o, k){
      o[k] = (keysToFix.indexOf(k) >= 0) ? parseInt(data[k], 10) : fixNumericValues(data[k]);
      return o;
    }, {});
  } else {
    return data;
...
P.makeRequest = function(method, loc, body, callback){
  ...
  var req = https.request(options, function(res){
    ...
    res.on('end', function(){
      try {
        callback(null, fixNumericValues(JSON.parse(chunks.join(''))));
      } catch(err) {
